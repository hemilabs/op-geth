...
func TestHistoryImportAndExport(t *testing.T) {
	...
	chain, err := core.NewBlockChain(db, nil, genesis, nil, ethash.NewFaker(), vm.Config{}, nil, nil, t.Context())
	if err != nil {
	...
	imported, err := core.NewBlockChain(db2, nil, genesis, nil, ethash.NewFaker(), vm.Config{}, nil, nil, t.Context())
	if err != nil {
		t.Fatalf("unable to initialize chain: %v", err)
	}
