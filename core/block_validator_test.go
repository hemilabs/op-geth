...
func testHeaderVerification(t *testing.T, scheme string) {
	...
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, nil, ethash.NewFaker(), vm.Config{}, nil, nil, t.Context())
	defer chain.Stop()
	if err != nil {
		t.Fatal(err)
	}
...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	// Run the header checker for blocks one-by-one, checking for both valid and invalid nonces
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, engine, vm.Config{}, nil, nil, t.Context())
	defer chain.Stop()
	if err != nil {
		t.Fatal(err)
	}
