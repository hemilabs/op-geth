...
func ProgressTip(ctx context.Context, currentTimestamp uint32) {
	...
	if tipHeight > uh+TBCTipHeightLag {
		...
		if endingHeight > uh {
			...
			pastHeaders, err := TBCIndexer.BlockHeadersByHeight(ctx, endingHeight)
			if err != nil {
				log.Info("Error getting block headers by height", "err", err)
				return
			}

			if len(pastHeaders) == 0 {
				log.Info("Could not find any headers", "height", endingHeight)
			}

			syncToHash := pastHeaders[0].BlockHash()

			TBCIndexer.SyncIndexersToHash(ctx, &syncToHash)
