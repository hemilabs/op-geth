...
// fetchHvmLightState is a blocking version of Peer.RequestHvmLightState which
...
func (d *Downloader) fetchHvmLightState(p *peerConnection, hash common.Hash) ([]*types.Header, *types.Block, error) {
	...
	req, err := p.peer.RequestHvmLightState(hash, resCh)
	if err != nil {
		return nil, nil, err
	}
	defer req.Close()
	...
	case <-timeoutTimer.C:
		...
		p.log.Debug("hVM light state request timed out", "elapsed", ttl)
		hvmLightStateTimeoutMeter.Mark(1)

		return nil, nil, errTimeout

	case res := <-resCh:
		// Headers successfully retrieved, update the metrics
		hvmLightStateReqTimer.Update(time.Since(start))

		// Don't reject the packet even if it turns out to be bad, downloader will
		// disconnect the peer on its own terms. Simply delivery the hVM light state to
		// be processed by the caller
		res.Done <- nil

		resParsed := res.Res.(*eth.HvmLightStateResponse)
		log.Info("Blocking hVM light state response received",
			"block", resParsed.Block.Number(), "headers", len(resParsed.Headers))

		return resParsed.Headers, resParsed.Block, nil
	}
}
